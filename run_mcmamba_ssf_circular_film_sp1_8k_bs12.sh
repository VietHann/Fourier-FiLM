...
EXP_ROOT="$ROOT/jnf_mamba_experiment/mcmamba_ssf_circular_film_experiment_20260729"
...
VERSION=mcmamba_ssf_circular_film_sp1_bs12_500ep
RUN_DIR="$EXP_ROOT/runs/8k/$VERSION"

mkdir -p "$RUN_DIR" "$EXP_ROOT/logs"
export PYTHONUNBUFFERED=1
export TMPDIR="$ROOT/.t_mcmamba_ssf_circular_film"
export CUDA_VISIBLE_DEVICES=0,1,2
mkdir -p "$TMPDIR"

exec "$ENV_BIN/python" -u "$EXP_ROOT/train_mcmamba_ssf_circular_film.py" \
  --sample-rate 8000 \
  --devices 3 \
